/* ════════════════════════════════════════════════════════════════════
   ...
   - Real campus images via curated Unsplash mapping
   - Loading states, skeleton loaders, graceful error handling
   - Card voting: remove on "Not for me", promote on "Good match"
   ════════════════════════════════════════════════════════════════════ */
...
/* ─── Campus Image Map ───────────────────────────────────────────────
   All images from Unsplash — hotlink-friendly, no key required.
   Key = lowercase substring matched against the school name.
   Falls back to a generated gradient placeholder if no match found.
   ─────────────────────────────────────────────────────────────────── */
const CAMPUS_IMAGES = {
...
/* ─── Major → Program Percentage Field Map ──────────────────────── */
const MAJOR_FIELDS = {
...
let currentQ     = 0;
const answers    = {};
let resultsData  = [];        // Final scored school objects
const compareSet = new Set(); // School IDs selected for comparison
let apiCache     = {};        // Cache Scorecard API responses
const upvoteOrder = [];       // Tracks order of "Good match" clicks for promotion ordering
...
function renderQuestion() {
  ...
  document.getElementById('btn-next').disabled = !answers[q.key];
...
function selectOption(value) {
  ...
  QUESTIONS[currentQ].opts.forEach((o, i) => {
    if (o.value === value)
      document.querySelectorAll('.option-item')[i].classList.add('selected');
  });
...
async function fetchColleges() {
  const params = new URLSearchParams({
    ...
    // Must have a valid admission rate
  ...
  // Size filter
  const sizeRanges = { small: '0..4000', medium: '4000..15000', large: '15000..200000' };
  if (sizeRanges[answers.size]) {
    params.append('latest.student.size__range', sizeRanges[answers.size]);
  }

  // Major program presence filter (at least 2% of students in the field)
  const majorField = MAJOR_FIELDS[answers.major];
  if (majorField) {
    params.append(majorField + '__range', '0.02..1');
  }

  // Sort by selectivity (mix of reach/target/safety in results)
...
/* ─── Normalize one raw API result into a clean school object ────── */
function normalizeSchool(raw) {
  ...
  const whitePct      = raw['latest.student.demographics.race_ethnicity.white'];
  ...
  // Vibe from program mix and school type
  ...
  const diversityScore = (whitePct !== null && whitePct !== undefined)
    ? Math.round((1 - whitePct) * 100)
    : 50;
  ...
  return {
    ...
    topPrograms,
    displayTuition,
    displayAcceptance: admissionRate ? Math.round(admissionRate * 100) + '%' : 'N/A',
    displayGradRate:   gradRate      ? Math.round(gradRate * 100) + '%'      : 'N/A',
    displayEarnings:   earnings      ? '$' + earnings.toLocaleString()       : 'N/A',
    displaySize:       studentSize   ? studentSize.toLocaleString() + ' students' : 'N/A',
  };
}
...
function scoreCollege(school) {
  ...
  // ── Major (30 pts) ──────────────────────────────────────────────
  const majorPctMap = {
    stem:        (school.programMap['Computer Science'] || 0)
               + (school.programMap['Engineering']      || 0)
               + (school.programMap['Biology/Life Sci'] || 0),
  ...
  if (school.sizeTier === answers.size) {
    ...
    const dist = Math.abs(
      sizeOrder.indexOf(school.sizeTier) - sizeOrder.indexOf(answers.size)
    );
  ...
  // ── Vibe (6 pts) ────────────────────────────────────────────────
  if (school.vibeTier === answers.vibe) {
    total += WEIGHTS.vibe;
  } else if (
    (answers.vibe === 'balanced' && school.vibeTier !== 'social') ||
  ...
  if (answers.diversity === 'not') {
    ...
    total += school.diversityScore >= 25
      ? WEIGHTS.diversity
      : Math.round(WEIGHTS.diversity * 0.6);
  }
...
/* ─── Gradient Placeholder (no image match) ──────────────────────── */
...
function buildFactorChips(school) {
  const chips = [];
  ...
  // Major
  const stemExtra = answers.major === 'stem'
    ? (school.programMap['Engineering'] || 0) + (school.programMap['Biology/Life Sci'] || 0)
    : 0;
  const majorPct = (school.programMap[majorLabels[answers.major]] || 0) + stemExtra;
  if (majorPct > 0.05) chips.push({ label: majorLabels[answers.major] + ' ✓', cls: 'factor-major' });

  // Region
  if (answers.region === 'any' || school.region === answers.region)
    chips.push({ label: 'Location ✓', cls: 'factor-region' });

  // Size
  if (school.sizeTier === answers.size)
    chips.push({ label: 'Size ✓', cls: 'factor-size' });

  // Budget
  if ((budgetOk[answers.budget] || []).includes(school.budgetTier))
    chips.push({ label: 'Budget ✓', cls: 'factor-budget' });

  // GPA
  const gpaDiff = (gpaRank[answers.gpa] ?? 1) - (gpaRank[school.gpaTier] ?? 1);
  chips.push(gpaDiff >= 0
    ? { label: gpaDiff >= 1 ? 'GPA Safety ✓' : 'GPA Fit ✓', cls: 'factor-gpa' }
    : { label: 'GPA Reach', cls: 'factor-gpa' });

  // Vibe
  if (school.vibeTier === answers.vibe)
    chips.push({ label: 'Vibe ✓', cls: 'factor-vibe' });

  // Weather
  if (answers.weather !== 'any' && school.weatherTier === answers.weather)
    chips.push({ label: 'Weather ✓', cls: 'factor-weather' });

  // Diversity
  if (answers.diversity === 'very' && school.diversityScore >= 50)
    chips.push({ label: 'Diverse Campus ✓', cls: 'factor-diversity' });

  // Internship / career
  if (answers.internship === 'very' && school.earnings > 50000)
    chips.push({ label: 'Career Outcomes ✓', cls: 'factor-internship' });
...
/* ─── Template-Based Match Explanation ───────────────────────────── */
function buildExplanation(school, pct, matchCategory) {
  const majorLabels = {
    stem:        'STEM programs',
    business:    'business programs',
    liberal_arts:'liberal arts programs',
    arts:        'arts programs',
  ...
  // Sentence 1: program + location fit
  const programNote = school.topPrograms.length
    ? `${school.name} stands out for its strength in ${school.topPrograms.slice(0, 2).join(' and ')}, which aligns with your interest in ${majorLabels[answers.major] || 'your chosen field'}.`
    : `${school.name} in ${school.location} aligns well with your interest in ${majorLabels[answers.major] || 'your field of study'}.`;

  // Sentence 2: GPA match level + key stat
  let gpaNote;
  if (gpaDiff >= 1) {
    gpaNote = `With your GPA, you are a strong candidate — this is a solid safety school with a ${school.displayGradRate} graduation rate.`;
  } else if (gpaDiff === 0) {
    gpaNote = `Your GPA closely matches the typical admitted student, making this a realistic target with median graduate earnings of ${school.displayEarnings}.`;
  } else {
    gpaNote = `This is a reach school given your GPA, but its ${school.displayAcceptance} acceptance rate and ${school.displayGradRate} graduation rate make it a worthwhile stretch goal.`;
  }
...
/* ═══════════════════════════════════════════════════════════════════
   MAIN ORCHESTRATOR: FETCH → SCORE → RENDER
   ═══════════════════════════════════════════════════════════════════ */
...
async function fetchAndShowResults() {
  ...
  document.getElementById('results-sub').textContent = apiWorked
    ? 'Powered by U.S. Department of Education College Scorecard data. Showing your top 5 personalized matches.'
    : 'Based on your academic profile and preferences. Results shown from our curated school database.';
...
function renderCards(scored, grid) {
  grid.innerHTML = '';
  ...
  scored.forEach((school, idx) => {
    ...
    const chips         = buildFactorChips(school);
    const explanation   = buildExplanation(school, pct, matchCategory);
      ...
      <div class="card-layout">
        ...
        <div class="card-content">
          ...
          <div class="why-section">
            ...
            <div class="why-text" id="why-text-${idx}">${explanation}</div>
...
/* ═══════════════════════════════════════════════════════════════════
   VOTING: "Good match" promotes card to top, "Not for me" removes it
   ═══════════════════════════════════════════════════════════════════ */

function vote(idx, dir) {
  const grid = document.getElementById('results-grid');
  const card = grid.querySelector(`.college-card[data-id="${resultsData[idx]?.id || idx}"]`);
  if (!card || card.dataset.voted) return; // Ignore if already voted
  card.dataset.voted = dir;

  if (dir === 'down') {
    // ── "Not for me" — slide out and remove ────────────────────────
    card.classList.add('card-removing');
    card.addEventListener('animationend', () => {
      card.remove();
      // Clean up compare set if this school was selected
      ...
      if (compareSet.has(schoolId)) {
        ...
        document.getElementById('btn-compare-bar').style.display =
          count >= 2 ? 'inline-flex' : 'none';
      }
    }, { once: true });

  } else {
    // ── "Good match" — gold-pulse up, then move to top of list ─────
    ...
    card.addEventListener('animationend', () => {
      ...
      // Place at the correct promoted position (chronological order)
      const rank         = upvoteOrder.indexOf(idx);
      ...
      if (rank === 0 || promotedCards.length <= 1) {
        ...
        if (sibling && sibling !== card) sibling.after(card);
        else grid.insertBefore(card, grid.firstChild);
      }

      // Trigger drop-in entrance after repositioning
      ...
      // Lock the upvote button as active
...
function openCompareModal() {
  const selected = resultsData.filter(s => compareSet.has(s.id || resultsData.indexOf(s)));
...
function renderCompareModal(schools) {
  ...
  const rows = [
    { label: 'Match Score',     fn: s => `<strong>${s.score}%</strong> — ${getMatchCategory(s.score, s)}` },
    { label: 'Location',        fn: s => s.location },
    { label: 'Tuition',         fn: s => s.displayTuition },
    { label: 'Avg Net Price',   fn: s => s.netPrice ? '$' + s.netPrice.toLocaleString() + '/yr' : 'N/A' },
    { label: 'Acceptance Rate', fn: s => s.displayAcceptance },
    { label: 'Graduation Rate', fn: s => s.displayGradRate },
    { label: 'Enrollment',      fn: s => s.displaySize },
    { label: 'Median Earnings', fn: s => s.displayEarnings },
    { label: 'Diversity Index', fn: s => s.diversityScore + '%' },
    { label: 'Top Programs',    fn: s => s.topPrograms.join(', ') || 'N/A' },
    { label: 'Campus Size',     fn: s => s.sizeTier.charAt(0).toUpperCase() + s.sizeTier.slice(1) },
    { label: 'Weather',         fn: s => s.weatherTier.charAt(0).toUpperCase() + s.weatherTier.slice(1) },
  ];
    ...
    <thead>
      <tr>
        ...
        ${schools.map(s => {
          ...
          return `<th>
            ${imgUrl
              ? `<img class="compare-school-img" src="${imgUrl}" alt="${s.name}"
  ...
  rows.forEach(row => {
    const values     = schools.map(s => row.fn(s));
    const numericVals = schools.map(s => {
      const raw = row.fn(s).replace(/<[^>]+>/g,'').replace(/[^0-9.]/g,'');
    ...
    html += `<tr>
      ...
      ${values.map((val, i) => {
        ...
        let cls = '';
        if (row.label === 'Acceptance Rate' && num === minVal && minVal !== maxVal)
          cls = 'compare-highlight'; // lower acceptance = more selective
        else if (['Match Score','Graduation Rate','Median Earnings','Diversity Index'].includes(row.label)
          && num === maxVal && maxVal !== minVal)
          cls = 'compare-highlight';
        else if (['Tuition','Avg Net Price'].includes(row.label)
          && num === minVal && minVal !== maxVal)
          cls = 'compare-highlight'; // lower cost = better
...
function getFallbackSchools() {
  return [
    ...
    // ── Midwest ──
    { id: 'uchicago',    name: 'University of Chicago',  city: 'Chicago',       state: 'IL', location: 'Chicago, IL',       website: 'uchicago.edu',       region: 'midwest',   sizeTier: 'medium', budgetTier: 'high',   gpaTier: 'high',     vibeTier: 'academic',  weatherTier: 'cold',    diversityScore: 55, admissionRate: 0.07, netPrice: 20000, ownership: 2, studentSize: 17000, gradRate: 0.95, earnings: 86000,  programMap: { 'Liberal Arts': 0.32, 'Social Sciences': 0.20 },                                   topPrograms: ['Liberal Arts','Social Sciences','Business'],          displayTuition: '$62,238',           displayAcceptance: '7%',  displayGradRate: '95%', displayEarnings: '$86,000',  displaySize: '17,000 students' },
